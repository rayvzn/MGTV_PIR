...
parser.add_argument("--model_dir", type=str, required=False, default="./export/", help="The directory to static model.")
...
if __name__ == "__main__":
    ...
    label_map = {0:"IP", 1:"明星", 2:"IP+明星", 3:"明星+频道", 4:"题材", 5:"other"}
    ...
    # text = '我是特种兵之火凤凰'
    texts = ['我是特种兵之火凤凰']
    data = [text[:args.max_seq_length] for text in texts]
    batches = [
    ...
    for _ in range(50):
        for batch_data in batches:
            predictor.predict(batch_data, tokenizer, label_map=label_map)

    import time
    start = time.time()
    for batch_data in batches:
        print('batch_data: ', batch_data)
        labels = predictor.predict(batch_data, tokenizer, label_map=label_map)
    end = time.time()
    print(labels)
    print("num data: %d, batch_size: %d, cost time: %.5f ms" %
          (len(data), args.batch_size, (end - start)*1000))
